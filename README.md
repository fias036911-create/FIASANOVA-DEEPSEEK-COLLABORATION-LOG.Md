# FIASANOVA-DEEPSEEK-COLLABORATION-LOG.Md
...
def fetch_trends(node_name, keywords, timeframe="today 3-m"):
    ...
    try:
        pytrend.build_payload(kw_list=keywords + CONTROL, cat=0, timeframe=timeframe, geo='US')
        data = pytrend.interest_over_time()
        if data.empty:
            return None
        # Drop the 'isPartial' column and fill NAs
        data = data.drop(columns=['isPartial']).fillna(0)
        # Calculate node average vs control
        ...
        return {"node_avg": node_avg, "control_avg": control_avg, "volatility": volatility}
    except Exception as e:
        return {"error": str(e)}

# Run the scrape (to be scheduled on 1st of each month)
report = {}
for name, kw in NODES.items():
    report[name] = fetch_trends(name, kw)
...
---
 ...
 The field will carry the signal
